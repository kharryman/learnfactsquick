...
export class CelebrityNumbersPage implements OnInit {
  ...
  namenums:any;
  @ViewChild('celebritiesResults') celebritiesResults:ElementRef;
  ...
  getCelebrityNumbers(){
    ...
    var eveyear = null;
    var text = "";
    var studynumcel:Boolean;
    var abbcelnam:Boolean;
    var nums = [];
    if (eveyear!=null)
    {
      console.log("getCelebrityNumbers eveyeyar NOT NULL");
      nums=eveyear.split(";");
      abbcelnam=true;
    }
    if (eveyear==null)
    {
      console.log("getCelebrityNumbers eveyear IS NULL");
      var studynumcel=this.isAllCelebrities;//document.getElementById("studynumcel").checked;//study celebrity numbers
      ...
      {
        console.log("getCelebrityNumbers studynumcel IS FALSE");
        var numsspl=this.celebrities.celebrityNumbers;
        console.log("getCelebrityNumbers this.celebrityNumbers=" + this.celebrities.celebrityNumbers);
        if(numsspl==null){
        ...
        nums=String(numsspl).split(";");
      }
      if (studynumcel==true)
      {
        console.log("getCelebrityNumbers studynumcel IS TRUE");
      ...
      text += "<b>";
      //text += "<u>";
      console.log("getCelebrityNumbers nums.length="+nums.length);
      for (var i=0;i<nums.length;i++)
      {
        ...
        if (studynumcel==false){text += disnumtex.bold().big() + k.toString().bold().big() + ": ";}
        for (var j=0;j<len;j++)
        {
    ...
    for (var z=0;z<nums.length;z++)
    {
      ...
      if (rem==0)
      {
        ...
        for(i=0;i<x;i++)//for every 2 numbers
        {
          ...
          {
            ind=ind.concat(String(numspl[db]-1));
          }
          if (numspl[db]==0)
          {
            ind=ind.concat(String(9));
          }
          if (numspl[dbp]!=0)
          {
            ind=ind.concat(String(numspl[dbp]-1));
          }
          if (numspl[dbp]==0)
          {
            ind=ind.concat(String(9));
    ...
    if (eveyear!=null)
    {
      ...
      this.celebritiesResults.nativeElement.innerHTML = text;
    }
  }
